...
public class OoniumOreBlock extends GrandcupModElements.ModElement {
	@ObjectHolder("grandcup:oonium_ore")
	public static final Block block = null;

	public OoniumOreBlock(GrandcupModElements instance) {
		super(instance, 47);
	...
	public static class CustomBlock extends Block {
		public CustomBlock() {
			super(Block.Properties.create(Material.SAND).sound(SoundType.SAND).hardnessAndResistance(1.45f, 10f).setLightLevel(s -> 0).harvestLevel(2)
					.harvestTool(ToolType.SHOVEL).setRequiresTool());
			setRegistryName("oonium_ore");
		}
	...
	private static class CustomRuleTest extends RuleTest {
		...
		public boolean test(BlockState blockAt, Random random) {
			...
			if (blockAt.getBlock() == Blocks.SANDSTONE)
				blockCriteria = true;
			if (blockAt.getBlock() == Blocks.CHISELED_SANDSTONE)
				blockCriteria = true;
			if (blockAt.getBlock() == Blocks.CUT_SANDSTONE)
				blockCriteria = true;
			if (blockAt.getBlock() == Blocks.SMOOTH_SANDSTONE)
				blockCriteria = true;
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			CUSTOM_MATCH = Registry.register(Registry.RULE_TEST, new ResourceLocation("grandcup:oonium_ore_match"), () -> CustomRuleTest.codec);
			...
			configuredFeature = feature.withConfiguration(new OreFeatureConfig(CustomRuleTest.INSTANCE, block.getDefaultState(), 5)).range(70)
					.square().func_242731_b(10);
			event.getRegistry().register(feature.setRegistryName("oonium_ore"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("grandcup:oonium_ore"), configuredFeature);
		}
	}
